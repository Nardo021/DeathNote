 ...
 */
public final class Forge189ItemBridge implements DeathNoteItemBridge<ItemStack> {
    ...
    public Forge189ItemBridge(DeathNoteConfig config, SignatureService signatures) {
        this.config = config;
    ...
    public ItemStack createDeathNoteItem(UUID creator) {
        ItemStack stack = new ItemStack(DeathNoteItem.INSTANCE);
    ...
    public boolean isDeathNoteItem(ItemStack item) {
        if (item == null || item.getItem() != DeathNoteItem.INSTANCE) {
            return false;
        }
