...
import com.yaazyy.deathnote.forge1165.audit.ForgeFileAuditSink;
import com.yaazyy.deathnote.forge1165.item.DeathNoteItems;
import com.yaazyy.deathnote.forge1165.item.Forge1165ItemBridge;
import com.yaazyy.deathnote.forge1165.network.DeathNoteNetwork;

import net.minecraftforge.fml.loading.FMLPaths;

/**
 * Holds wired Death Note services for the Forge 1.16.5 mod.
 */
public final class ModContext {
    ...
    private Forge1165ItemBridge itemBridge;
    private Forge1165DeathNotePlatform platform;
    ...
    public static void init(Logger logger) {
        File dir = FMLPaths.CONFIGDIR.get().resolve("deathnote").toFile();
    ...
    private void copyIfMissing(String name) {
        ...
        try (InputStream in = ModContext.class.getResourceAsStream("/" + name)) {
            if (in == null) {
                logger.warning("Missing bundled default: " + name);
                return;
            }
            Files.copy(in, target.toPath());
        } catch (IOException e) {
            logger.warning("Failed to copy default " + name + ": " + e.getMessage());
        }
    }
    ...
    private void wire() {
        try {
            ...
            logger.severe("Failed to load config, using defaults: " + e.getMessage());
            this.config = DeathNoteConfig.defaults();
        }

        this.signatures = new SignatureService(config.security().hmacSecret);
        if (DeathNoteItems.DEATH_NOTE != null) {
            this.itemBridge = new Forge1165ItemBridge(config, signatures, DeathNoteItems.DEATH_NOTE);
        }
        this.platform = new Forge1165DeathNotePlatform(config);
    ...
    public void bindDeathNoteItem(net.minecraft.item.Item item) {
        this.itemBridge = new Forge1165ItemBridge(config, signatures, item);
    }

    public Forge1165ItemBridge itemBridge() {
        if (itemBridge == null) {
            throw new IllegalStateException("Death Note item not registered yet");
        }
        return itemBridge;
    }

    public Forge1165DeathNotePlatform platform() {
        return platform;
    }
