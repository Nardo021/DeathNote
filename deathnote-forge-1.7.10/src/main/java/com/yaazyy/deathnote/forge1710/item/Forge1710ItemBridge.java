 ...
 */
public final class Forge1710ItemBridge implements DeathNoteItemBridge<ItemStack> {
    ...
    public Forge1710ItemBridge(DeathNoteConfig config, SignatureService signatures, Item deathNoteItem) {
        this.config = config;
    ...
    public boolean isDeathNoteItem(ItemStack item) {
        if (item == null || item.getItem() != deathNoteItem) {
            return false;
        }
    ...
    private NBTTagCompound tagOf(ItemStack item) {
        if (item == null || item.getItem() != deathNoteItem || !item.hasTagCompound()) {
            return null;
        }
